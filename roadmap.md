...
❯ У меня есть несколько направлений, в которых может пойти дальнейшее развитие этого проекта.                                                                                   
1. Личная "песочница" кода. Запуск проекта с авторизацией от моего лица или с другими способоами аутентификации - на итог я хочу из браузера получать доступ к терминалу и      
виртуальному контейнеру. Я изучаю несколько языков программирования: rust, go, haskell. Я хочу для каждого из них поднять и держать "горячим" запущенный виртуальный контейнер  
с настроенной средой и тулингом соответствующего языка. Например, на rust есть набор упражнений для начинающих: rustlings. Я хочу, чтобы у меня была возможность практиковаться 
 в нем из любого места.                                                                                                                                                         
2. Второе направление - это, по сути, масштабирование первого, уже не для индивидуального использования, а для пердоставления бесплатного сервиса неограниченному кругу лиц, но 
 в лимитированных объемах. Например, возьмем тот же пример с rustlings. Я хочу держать наготове три "горячих" запущенных контейнера, в режиме ожидания, с установленным rust. И 
 сделать веб-сервис: на страничке которого будет, например, кнопка "запустить терминал". Нажимая которую, пользователь открывает сессию одного из находящихся в резерве         
контейнеров. Пользователь проводит сессию работы - после чего при окончании сессии вся информация которую он ввел - удаляется вместе с контейнером. Пока пользователь работает  
- у нас остаются еще два резервных контейнера, и еще два пользователя могут начать сессию работы. Если все контейнеры заняты - пользователь получит отказ "Времеенно нет        
свободных ресурсов, попробуйте позднее". После завершения - контейнер пользователя удаляется, и поднимается новый контейнер, который пополняет резерв.                          
                                                                                                                                                                                
rustlings - это набор из 92 упражнений. Возможно, вариант, когда на веб-странице будут ссылки на номера конкретных упражнений, и при запуске контейнера пользователем внутрь    
контейнера будет передаваться скрипт, сразу запускающий нужное упражнение. (вся вышеописанная логика при этом сохраняется).                                                     
3. Третье направление - платная подписака для авторизованных пользователей. Они будут получать некоторый volume на хостинге, где поднимаются контейнеры - и на них не будет     
распространяться ограничение по доступным в данное время контейнерам - они будут получать услугу в любом случае, даже если лимита свободных контейнеров нет. Данные и прогресс  
таких пользователей будут сохраняться.                                                                                                                                          
                                                                                                                                                                                
Число три здесь - условное, зависит от мощности хостинга. Вполне возможно, что вместо трех будет десять контейнеров.                                                            
                                                                                                                                                                                
Я вижу это как микросервисную архитектуру - сервис контейнеров должен работать отдельно и иметь возможность встраиваться на статическую страничку через ссылку в html как       
виджет. Виджет будет на js: он будет управлять логикой нажатия кнопок, внешним видом и прочим. Но сам сервис - должен быть независим, отдельно, и иметь возможность             
реализовывать все вышеперечисленные варианты. В случае с авторизованными пользователями - возможно я сделаю платформу на Django. В случае с неавторизованными пользователями,   
или с личной страничкой - я планирую использовать статические сайты на бесплатном хостинге, например GitHub Pages, и вставлять виджет в статическую страничку (сам микросервис  
будет располагаться на отдельно выделенном под него сервере).
